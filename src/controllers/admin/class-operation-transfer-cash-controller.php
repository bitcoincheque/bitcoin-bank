 ...
 * Description.
 */
class Operation_Transfer_Cash_Controller extends Admin_Controller
{
    ...
    {
        $model_class = null;
        $view_class = 'BCQ_BitcoinBank\Operation_Transfer_Cash_View';
    ...
    public function button_transfer_type_reload_cheque_click()
    {
        ...
        $hot_vault_account = Accounting::get_sub_account_chart_list(Account_Chart_Db_Table::SUB_TYPE_BALANCE_ASSET_CASH_HOT_VAULT);
        $cold_vault_account = Accounting::get_sub_account_chart_list(Account_Chart_Db_Table::SUB_TYPE_BALANCE_ASSET_CASH_COLD_VAULT);

        $all_accounts = array_merge($hot_vault_account, $cold_vault_account);
        ...
        foreach ($all_accounts as $account) {
            ...
            $item_list[$account_id] = $item;
        }
        $this->view->from_account->set_items($item_list);
        $this->view->to_account->set_items($item_list);
    ...
    public function button_make_transfer_click()
    {
        ...
        if (is_user_logged_in())
        {
            ...
            if ( ! $errors )
            {
                $transaction_id = Accounting::make_account_transfer($from_account, $to_account, $amount_satoshi, null, true);
