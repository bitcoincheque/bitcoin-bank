...
class Operation_Add_Cash_View extends Admin_Std_View {
    ...
    public function create_content( $parameters = null ) {
        ...
        $grid->add_row();
        $grid->add_cell_header( new Label( 'Cash vault:', array( 'for' => 'from_account' )) );
        $grid->add_cell( $this->from_account );

        $grid->add_row();
        $grid->add_cell_header( new Label( 'To client account:', array( 'for' => 'to_account' )) );
