...
class Operation_Transfer_Cash_View extends Admin_Std_View {
    ...
    public function create_content( $parameters = null ) {
        ...
        $this->add_content($this->button_transfer_type_reload_cheque);

        $grid = new Grid(null, array('class' => 'form-table'));

        $grid->add_row();
        $grid->add_cell_header( new Label( 'From vault:', array( 'for' => 'from_account' )) );
        $grid->add_cell( $this->from_account );

        $grid->add_row();
        $grid->add_cell_header( new Label( 'To vault:', array( 'for' => 'to_account' )) );
