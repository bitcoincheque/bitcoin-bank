...
class Wp_User_Id_Type extends Id_Type {

    public function get_formatted_text() {
        $user_data = get_userdata($this->value);
        if($user_data) {
            $text = $user_data->user_login;
        } else {
            $text = 'Invalid user ' . strval($this->value);
        }
        return $text;
    }
    ...
    public function create_content() {
        ...
        {
            $href = get_site_url() . '/wp-admin/user-edit.php?user_id=' . strval($this->value);
            $a = new A($text, $href);
            $this->set_content($a);
        } else {
